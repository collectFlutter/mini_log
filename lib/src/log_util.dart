...
import 'mini_log_db_manage.dart';
import 'model.dart';

import 'mini_log_config.dart';

class L {
  /// 日志配置
  static MiniLogConfig _config = MiniLogConfig();

  /// 初始化日志
  static void init([MiniLogConfig config]) {
  ...
  static void V(Object object, {String tag, bool withSQLite, bool withUp}) {
    if (MiniLogLevelEnum.V >= _config.minPrintLevel) {
      _handleLog(MiniLogLevelEnum.V, object, tag: tag, withSQLite: withSQLite, withUp: withUp);
    }
  }

  /// Info来表达一些信息。
  static void I(Object object, {String tag, bool withSQLite, bool withUp}) {
    if (MiniLogLevelEnum.I >= _config.minPrintLevel) {
      _handleLog(MiniLogLevelEnum.I, object, tag: tag, withSQLite: withSQLite, withUp: withUp);
    }
  }

  ///蓝色，Debug来表达调试信息。
  static void D(Object object, {String tag, bool withSQLite, bool withUp}) {
    if (MiniLogLevelEnum.D >= _config.minPrintLevel) {
      _handleLog(MiniLogLevelEnum.D, object, tag: tag, withSQLite: withSQLite, withUp: withUp);
    }
  }

  /// Warn表示警告，但不一定会马上出现错误，开发时有时用来表示特别注意的地方。
  static void W(Object object, {String tag, bool withSQLite, bool withUp}) {
    if (MiniLogLevelEnum.W >= _config.minPrintLevel) {
      _handleLog(MiniLogLevelEnum.W, object, tag: tag, withSQLite: withSQLite, withUp: withUp);
    }
  }

  ///Error 出现错误，是最需要关注解决的。
  static void E(Object object, {String tag, bool withSQLite, bool withUp}) {
    if (MiniLogLevelEnum.E >= _config.minPrintLevel) {
      _handleLog(MiniLogLevelEnum.E, object, tag: tag, withSQLite: withSQLite, withUp: withUp);
    }
  }

  static Future<List<MiniLogModel>> queryLogs([QueryLogParameter _parameter]) async {
    if (_config.withSQLite ?? false)
      return await MiniLogDBManage.internal().query(_parameter ?? QueryLogParameter());
    return [];
  }

  static Future<int> deleteLog([QueryLogParameter _parameter]) async {
    return await MiniLogDBManage.internal().delete(_parameter ?? QueryLogParameter());
  }
  ...
  static void _handleLog(
    ...
    if (withUp) {
      _config.upLogEvent(_log).then((value) {
        ...
        if (withSQLite) {
          MiniLogDBManage.internal().insert(_log);
        }
      });
    } else {
      if (withSQLite) {
